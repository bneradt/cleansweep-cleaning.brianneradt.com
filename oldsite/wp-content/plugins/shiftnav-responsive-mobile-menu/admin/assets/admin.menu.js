jQuery(document).ready(function ($) {
	...
	$("#menu-management").on(
		"mouseenter touchEnd MSPointerUp pointerup",
		".menu-item:not(.shiftnav-processed)",
		function (e) {
			$(this).addClass("shiftnav-processed");
			$(this)
				.find(".item-title")
				.append(
					'<span class="shiftnav-settings-toggle" data-shift-toggle="' +
						$(this).attr("id") +
						'"><i class="fa fa-gear"></i> Shift <span class="shiftnav-unsaved-alert"><i class="fa fa-warning"></i> <span class="shiftnav-unsaved-alert-message">Unsaved</span></span></span>'
				);
		}
	);
	...
	$("#menu-management").on(
		...
		function (e) {
			...
			//Switching to a different item
				...
				$current_panel = $settingswrap.find(
					".shiftnav-menu-item-panel-" + this_menu_item_id
				);

				//Create Panel if it doesn't exist
				if ($current_panel.length === 0) {
					$current_panel = $(
						".shiftnav-menu-item-panel-negative"
					).clone();
					$current_panel.removeClass(
						"shiftnav-menu-item-panel-negative"
					);
					$current_panel.addClass(
						"shiftnav-menu-item-panel-" + this_menu_item_id
					);
					$current_panel.attr(
						"data-menu-item-target-id",
						this_menu_item_id_num
					);

					var hash = "#" + this_menu_item_id;

					$current_panel
						.find(".shiftnav-menu-item-title")
						.text(
							$current_menu_item.find(".menu-item-title").text()
						);
					$current_panel
						.find(".shiftnav-menu-item-id")
						.html('<a href="' + hash + '">' + hash + "</a>");
					$current_panel
						.find(".shiftnav-menu-item-type")
						.text($current_menu_item.find(".item-type").text());
					var item_data =
						shiftnav_menu_item_data[this_menu_item_id_num];
					if (item_data) {
						$current_panel
							.find("[data-shiftnav-setting]")
							.each(function () {
								var _data_name =
									$(this).data("shiftnav-setting");
								...
								switch (_data_name) {
									case "icon":
										var $icon_wrap = $(this).parents(
											".shiftnav-icon-settings-wrap"
										);
										//console.log( item_data.icon );
										$icon_wrap
											.find(".shiftnav-icon-selected i")
											.attr("class", item_data.icon);
										break;
								}
							});
					...
					$current_panel
						.find(".shiftnav-menu-item-tab-content")
						.hide();
					...
					$current_panel.on(
						...
						function (e) {
							...
							//
							$current_panel
								.find(".shiftnav-menu-item-tab > a")
								.removeClass("shiftnav-menu-item-tab-current");
							...
							$current_panel
								.find(
									'[data-shiftnav-tab-content="' +
										$(this).data("shiftnav-tab") +
										'"]'
								)
								.slideDown();
	...
	$settingswrap.on(
		"change",
		".shiftnav-menu-item-setting-input",
		function (e) {
			//Flag Settings Panel
			var $form = $(this).parents(
				"form.shiftnav-menu-item-settings-form"
			);
			$form
				.find(".shiftnav-menu-item-status")
				.attr(
					"class",
					"shiftnav-menu-item-status shiftnav-menu-item-status-warning"
				);
			$form
				.find(".shiftnav-status-message")
				.html(
					"Settings have changed.  Click <strong>Save Menu Item</strong> to preserve these changes."
				);

			//Flag Menu Item
			var item_id = $form
				.parents(".shiftnav-menu-item-panel")
				.data("menu-item-target-id");
	...
	$settingswrap.on("click", ".shiftnav-menu-item-save-button", function (e) {
		...
		$formStatus.attr(
			"class",
			"shiftnav-menu-item-status shiftnav-menu-item-status-working"
		);
		$formStatusMessage.text("Processing save request...");

		$.post(
			shiftnav_meta.ajax_url,
			data,
			function (response) {
				...
				if (response == -1) {
					$formStatus.attr(
						"class",
						"shiftnav-menu-item-status shiftnav-menu-item-status-error"
					);
					$formStatusMessage.html(
						"<strong>Error encountered.  Settings could not be saved.</strong>  Your login/nonce may have expired.  Please try refreshing the page."
					);
					...
					$formStatus.attr(
						"class",
						"shiftnav-menu-item-status shiftnav-menu-item-status-success"
					);
			...
			$formStatusMessage.html(
				"<strong>Error encountered.  Settings could not be saved.</strong>  Response Text: <br/><textarea>" +
					d.responseText +
					"</textarea>"
			);
	...
	$settingswrap.on(
		"click",
		".shiftnav-icon-settings-wrap .shiftnav-icon-wrap",
		function (e) {
			$icon = $(this).find(".shiftnav-icon");
			$icon_set = $(this).parents(".shiftnav-icon-settings-wrap");
			// console.log(
			// 	$icon.attr("class") + " | " + $icon.data("shiftnav-icon")
			// );
			$icon_set
				.find(".shiftnav-icon-selected i")
	...
	$settingswrap.on("keyup", ".shiftnav-icons-search", function (e) {
		$icon_set = $(this)
			.parents(".shiftnav-icon-settings-wrap")
			.find(".shiftnav-icon-wrap");
		var val = $(this).val();
		if (val == "") {
			$icon_set.show();
		} else {
			$icon_set
				.filter(":not( [data-shiftnav-search-terms*=" + val + "] )")
				.hide();
			// console.log(
			// 	"not( [data-shiftnav-search-terms*=" +
