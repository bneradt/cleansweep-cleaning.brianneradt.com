...
function shiftnav_get_settings_fields()
{
	...
	$fields = array(
		...
		$prefix . 'togglebar' => array(
			...
			60 => array(
				...
				'desc' => __('The area which will trigger the ShiftNav Panel to open.  (Only relevant for Full Bar toggle style)'),
				'type' => 'radio',
				'options' => array(
			...
			160 => array(
				'name' => 'togglebar_injection',
				'label' => __('Toggle Bar Injection', 'shiftnav'),
				'desc' => __('Inject the toggle bar HTML at the beginning of the body HTML, or in the footer.  The bar will still appear at the top either way, this just affects the markup order.  Header is generally better if your theme supports the wp_body_open hook', 'shiftnav'),
				'type' => 'radio',
				'options' => array(
					'header' => __('Header', 'shiftnav'),
					'footer' => __('Footer', 'shiftnav'),
				),
				'default' => 'header',
			),
	...
	$fields[$prefix . 'general'] = array(
		...
		20 => array(
			...
			'desc' => __('Shift the body of the site when the menu is revealed.  For some themes, this may negatively affect the site content, so this is disabled by default.', 'shiftnav'),
			'type' => 'checkbox',
			'default' => 'off'
		),
		...
		210 => array(
			...
			'desc' => __('By default the tag is a button for accessibility.  Note that some themes will try to add JS to an anchor and break the back button functionality, so you can also switch this to a span to avoid the issue if needed.  In general, recommended setting is Button.', 'shiftnav'),
			'type' => 'radio',
			'options' => array(
				'button' => '&lt;button&gt;',
				'a' => '&lt;a&gt;',
...
{
	?>
	<a class="button" href="<?php echo admin_url('themes.php?page=shiftnav-settings'); ?>">&laquo; Back to ShiftNav Control
		Panel</a>
	<?php
}
...
function shiftnav_settings_panel()
{
	...
	<div class="wrap shiftnav-wrap">
		...
		<h1>ShiftNav <?php if (SHIFTNAV_PRO)
			echo 'Pro <i class="fa fa-rocket"></i>'; ?> <span class="shiftnav-version">v<?php echo SHIFTNAV_VERSION; ?></span>
		</h1>
...
function shiftnav_check_menu_assignment()
{
	...
	if ($display == 'on') {
		if (!has_nav_menu('shiftnav')) {
			?>
			<div class="update-nag"><strong>Important!</strong> There is no menu assigned to the <strong>ShiftNav [Main]</strong>
				Menu Location. <a href="<?php echo admin_url('nav-menus.php?action=locations'); ?>">Assign a menu</a></div>
			<br /><br />
			<?php
		}
	}
