...
//if( $item->object == 'ubermenu-custom' ) shiftp( $item );
		...
		if (SHIFTNAV_PRO) {
			...
			$this->submenu_type = $submenu_type = 'always';
		...
		if ($disable_link)
			unset($atts['href']);			//remove href for disabled links
		...
		if (isset($data['anchor_class']) && ($data['anchor_class']))
			$anchor_class .= ' ' . esc_attr($data['anchor_class']);
		...
		if ($icon)
			$title = '<span class="shiftnav-target-text">' . $title . '</span>';
	...
	function getUberOption($item_id, $id)
	{
		...
		if (!isset($this->menuItemOptions[$item_id])) {
			...
			if (empty($this->noUberOps[$item_id])) {
				...
				if (!$uber_options)
					$this->noUberOps[$item_id] = true; //don't check again for this menu item
			}
