...
<div style="display: none;" class="contact-info footer-1">	

<?php } // end if hide ?>	
	<a name="contact"></a> 
	<div class="footer-col-title" style="color: <?php echo get_theme_mod( 'footer-title-color' ); ?> !important;"><?php  echo get_theme_mod( 'contact-form-title', 'Contact Us' ); ?></div>

	<div class="business-info-item business-name"><?php if ( get_theme_mod( 'yext-id' )) { ?><span data-yext-field="locationName" data-yext-id="<?php echo get_theme_mod( 'yext-id' ); ?>"><?php } // end if yext id ?><?php echo get_theme_mod( 'business-name' ); ?><?php if ( get_theme_mod( 'yext-id' )) { ?></span><?php } // end if yext id ?>
	</div>
		
	<div class="business-info-item phone-number"><span class="contacts">Phone: </span><a <?php if ( get_theme_mod( 'yext-id' )) { ?> data-yext-field="phone" data-yext-id="<?php echo get_theme_mod( 'yext-id' ); ?>" <?php } // end if yext id ?> href="tel:<?php echo get_theme_mod( 'phone-number', '888-555-1234' ); ?>" <?php if ( get_theme_mod( 'phone-number' ) ) { 
				echo 'aria-label="Call ';
				$number = get_theme_mod( 'phone-number' );
				$formatNUM = str_replace('( ', '', implode(' ',str_split(str_replace(' ', '', $number))));
				$ariaNumber = str_replace(array(' -',' )'), '.', $formatNUM);
				echo $ariaNumber . '"';
			} ?>><?php echo get_theme_mod( 'phone-number', '888-555-1234' ); ?></a></div>	
	<div class="business-info-item email"><span class="contacts">Email: </span><a  href="mailto:<?php echo get_theme_mod( 'email', 'name@domain.com' ); ?>"><?php echo get_theme_mod( 'email', 'name@domain.com' ); ?></a></div>
	<div class="business-info-item address">
		<span class="contacts">Address: </span><?php if ( get_theme_mod( 'yext-id' )) { ?><span data-yext-field="address1" data-yext-id="<?php echo get_theme_mod( 'yext-id' ); ?>"><?php } // end if yext id ?><?php echo get_theme_mod( 'address-street','1234 Main St.' ); ?>
		<?php if ( get_theme_mod( 'yext-id' )) { ?></span><?php } // end if yext id ?><br><?php if ( get_theme_mod( 'yext-id' )) { ?><span data-yext-field="city" data-yext-id="<?php echo get_theme_mod( 'yext-id' ); ?>"><?php } // end if yext id ?><?php echo get_theme_mod( 'address-city','Citytown' ); ?><?php if ( get_theme_mod( 'yext-id' )) { ?></span><?php } // end if yext id ?><?php if ( get_theme_mod( 'address-city' )) { ?>,<?php } // end if field filled out ?> <?php if ( get_theme_mod( 'yext-id' )) { ?><span data-yext-field="state" data-yext-id="<?php echo get_theme_mod( 'yext-id' ); ?>"><?php } // end if yext id ?><?php echo get_theme_mod( 'address-state','NY' ); ?><?php if ( get_theme_mod( 'yext-id' )) { ?></span><?php } // end if yext id ?> <?php if ( get_theme_mod( 'yext-id' )) { ?><span data-yext-field="zip" data-yext-id="<?php echo get_theme_mod( 'yext-id' ); ?>"><?php } // end if yext id ?><?php echo get_theme_mod( 'address-zip','12345' ); ?><?php if ( get_theme_mod( 'yext-id' )) { ?></span><?php } // end if yext id ?>
	</div>	
		
	<?php if ( get_theme_mod( 'footer_social' ) == 'address' ) { ?>
		<div title="Social Media Links" class="social bottom-bar-social">	
